...
export default function EditApp({
  ...
  const onSubmit = (e:any) => {
    ...
    if(objFormData && selectedAppIndex) {
      ...
      let newAppArr = [...appointments];

      newAppArr[selectedAppIndex] = finalObj;


      setAppointments(newAppArr);
  ...
  const onCancel = () => {
    if(selectedAppIndex) {
      let newAppArr = [...appointments];
      newAppArr.splice(selectedAppIndex, 1);
  ...
  return (
    <form className="p-5 mt-4 pb-10" onSubmit={onSubmit}>
        ...
        {clinics.map((cl:any, idx:number) => (
          <div className="border border-gray1 p-2 rounded-lg">
            ...
            <div className='flex items-center gap-3'>
              <input type="radio" name="veterinary_name" id={`vet_name_${idx}`} value={cl.veterinary_name} checked={cl.veterinary_name === selectedApp.veterinary.veterinary_name} required/>
      ...
      <select name="services" className="border border-gray1 px-5 py-2 rounded-lg">
        <option value="Consultation" selected={selectedApp.services === "Consultation"}>Consultation</option>
        <option value="Vaccination" selected={selectedApp.services === "Vaccination"}>Vaccination</option>
      </select>
      ...
      <div className="flex w-full gap-10">
        <div className="w-1/4">
          ...
          <input type="text" className="text-gray1 w-full search-input" name="pet_name" defaultValue={selectedApp.pet.pet_name}/>
        </div>
        <div className="w-1/4">
          <p className="font-semibold text-black1 mb-4 ">Pet Breed</p>
          <input type="text" className="text-gray1 w-full search-input" name="pet_breed" defaultValue={selectedApp.pet.breed}/>
        </div>
        <div className="w-1/4">
          <p className="font-semibold text-black1 mb-4 ">Pet Age</p>
          <input type="text" className="text-gray1 w-full search-input" name="pet_age" defaultValue={selectedApp.pet.age}/>
        </div>
        <div className="w-1/4">
          ...
          <select name="pet_gender" className="border border-gray1 px-5 py-2 rounded-lg">
            <option value="Male" selected={selectedApp.pet.gender === "Male" ? true : false}>Male</option>
            <option value="Female" selected={selectedApp.pet.gender === "Female" ? true : false}>Female</option>
          </select>
        </div>
      </div>
      ...
      <div className="flex w-full gap-10">
        <div className="w-1/4">
          ...
          <input type="text" className="text-gray1 w-full search-input" name="owner_name" defaultValue={selectedApp.owner_name}/>
        </div>
        <div className="w-1/4">
          <p className="font-semibold text-black1 mb-4 ">Owner Email</p>
          <input type="email" className="text-gray1 w-full search-input" name="owner_email" defaultValue={selectedApp.owner_email}/>
        </div>
        <div className="w-1/4">
          <p className="font-semibold text-black1 mb-4 ">Owner Phone</p>
          <input type="text" className="text-gray1 w-full search-input" name="owner_phone" defaultValue={selectedApp.owner_phone}/>
        </div>
        <div className="w-1/4">
          <p className="font-semibold text-black1 mb-4 ">Owner Address</p>
          <input type="text" className="text-gray1 w-full search-input" name="owner_address" defaultValue={selectedApp.owner_address}/>
        </div>
      </div>
      ...
      <div className="flex justify-between mt-12 gap-3">
        <div>
          <button className="text-white bg-orange1 py-3 px-4 rounded-xl" type="submit" onClick={onCancel}>
            Cancel Appointment
          </button>
        </div>
        <div className="flex gap-3">
          <button className="text-black1 border border-gray1 py-3 px-4 rounded-xl" onClick={() => setShowEditApp(false)}>
            Cancel
          </button>
